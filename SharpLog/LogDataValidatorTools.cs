...
namespace SharpLog
{
    ...
    public static class LogDataValidatorTools
    {
        ...
        public static bool IsValidity(string callSign, string frequency)
        {
            //判定呼号和频率是否都合法
            if (HasIllegalCharacters(callSign, out string callSignErrorText) && (!IsValidAmateurFrequency(frequency, out string frequencyErrorText)))
            {
                MessageBox.Show($"呼号和频率填写错误\r\n呼号错误：{callSignErrorText}\r\n频率错误：{frequencyErrorText}", "呼号和频率错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            //判定呼号是否合法
            else if (HasIllegalCharacters(callSign, out callSignErrorText))
            {
                MessageBox.Show($"呼号包含非法字符：{callSignErrorText}", "呼号格式错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            //判定频率是否合法
            else if (!IsValidAmateurFrequency(frequency, out frequencyErrorText))
            {
        ...
        /// <returns>true=包含非法字符，false=仅含合法字符</returns>
        ...
        private static bool IsValidAmateurFrequency(string frequencyInMhz, out string FreErrorText)
        {
            ...
            {
                FreErrorText = $"错误：输入'{frequencyInMhz}'不是有效的数字格式";
                return false;
            }
            ...
            if (frequencyValue <= 0)
            {
                ...
                FreErrorText = $"错误：频率'{frequencyValue}MHz'不能为零或负数";
